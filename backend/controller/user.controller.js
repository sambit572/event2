...
import {ApiResponse} from "../utilities/ApiResponse.js"; // Corrected: Default import (no curly braces)
import jwt from "jsonwebtoken";
import { isValidPhoneNumber } from 'libphonenumber-js'; // This is a named import, so curly braces are correct here.

// Option for cookies (consider 'secure: true' in production with HTTPS)
const option = {
  httpOnly: true,
...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    // These methods (generateAccessToken, generateRefreshToken) are expected to be defined
    // on your User Mongoose schema in user.model.js
    const accessToken = await user.generateAccessToken();
    const refreshToken = await user.generateRefreshToken();

    user.refreshToken = refreshToken; // Store refresh token in user document
    await user.save({ validateBeforeSave: false }); // Save without re-running validation

    return { accessToken, refreshToken };
  } catch (error) {
    console.error(`Error in generating token: ${error}`);
    // Return an object indicating error to be handled by caller
...
const loginUser = async (req, res) => {
  try {
    ...
    return res.status(500).json(new ApiError(500, "Internal Server Error during login."));
  }
};
...
const resetPassword = async (req, res) => {
  try {
    ...
    const { resetToken } = req.params; // The unhashed token from the URL
