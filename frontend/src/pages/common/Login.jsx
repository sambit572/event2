...
const Login = ({ onClose }) => {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    try {
      ...
      if (err.response?.data?.message === "Phone number is required for new users") {
  ...
  const handleGooglePhoneSubmit = async (e) => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || "Google signup failed. Try again.";
      setErrorMsg(msg);
    }
  };
  ...
  const renderStep = () => {
    ...
    if (step === "otp") 
      return <OTPVerification setStep={setStep} />;
