...
const found = Array.isArray(res.data) && res.data.some((item) => item.service?._id === serviceId);
...
try {
await axios.post(`${BACKEND_URL}/wishlist/toggle/${serviceId}`, {}, { withCredentials: true });
const newStatus = !isWishlisted;
setIsWishlisted(newStatus);
toast.success(newStatus ? "Added to wishlist!" : "Removed from wishlist!");
window.dispatchEvent(new CustomEvent("wishlistUpdated", { detail: { serviceId } }));
} catch (err) {
...
switch (platform) {
case 'facebook':
shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(serviceUrl)}`;
break;
case 'twitter':
shareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(serviceUrl)}`;
break;
case 'whatsapp':
shareUrl = `https://wa.me/?text=${encodeURIComponent(shareText + ' ' + serviceUrl)}`;
break;
...
return;
case 'telegram':
shareUrl = `https://t.me/share/url?url=${encodeURIComponent(serviceUrl)}&text=${encodeURIComponent(shareText)}`;
break;
case 'copy':
navigator.clipboard.writeText(serviceUrl).then(() => {
...
try {
await axios.post(`${BACKEND_URL}/cart/add`, { serviceId }, { withCredentials: true });
...
await axios.post(`${BACKEND_URL}/notifications/notify-when-available`, { serviceId }, { withCredentials: true });
...
onClick={handleShare}
>
<img src="/send.png" alt="Share" className="h-full w-full rounded-full object-cover" />
</div>

<div className={`absolute top-full right-0 mt-2 min-w-[160px] origin-top-right rounded-xl border border-gray-200 bg-white shadow-2xl transition-all duration-200 ease-out ${showShareMenu ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
<div className="py-2">
<div className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm text-gray-800 transition-colors duration-200 ease-in-out hover:bg-gray-50" onClick={() => shareService('facebook')}>
<img src="/facebook.png" alt="Facebook" className="h-5 w-5 object-contain" /> Facebook
</div>
<div className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm text-gray-800 transition-colors duration-200 ease-in-out hover:bg-gray-50" onClick={() => shareService('twitter')}>
<img src="/twitter 1.png" alt="X" className="h-5 w-5 object-contain" /> X
</div>
<div className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm text-gray-800 transition-colors duration-200 ease-in-out hover:bg-gray-50" onClick={() => shareService('whatsapp')}>
<img src="/whatsapp.png" alt="WhatsApp" className="h-5 w-5 object-contain" /> WhatsApp
</div>
{/* ADDED INSTAGRAM BUTTON */}
<div className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm text-gray-800 transition-colors duration-200 ease-in-out hover:bg-gray-50" onClick={() => shareService('instagram')}>
<img src="/instagram.png" alt="Instagram" className="h-5 w-5 object-contain" /> Instagram
</div>
<div className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm text-gray-800 transition-colors duration-200 ease-in-out hover:bg-gray-50" onClick={() => shareService('telegram')}>
<img src="/telegram.png" alt="Telegram" className="h-5 w-5 object-contain" /> Telegram
</div>
<div className="flex cursor-pointer items-center gap-3 px-4 py-3 text-sm text-gray-800 transition-colors duration-200 ease-in-out hover:bg-gray-50" onClick={() => shareService('copy')}>
<img src="/connection.png" alt="Copy Link" className="h-5 w-5 object-contain" /> Copy Link
</div>
</div>
</div>
</div>
...
<div>
<Link to={`/service/${serviceId}`} className="text-inherit no-underline pr-12">
<h3 className="text-lg font-bold leading-tight text-[#2c3e50] sm:text-xl md:text-2xl">{title}</h3>
</Link>
<div className="mt-3 mb-3 flex flex-wrap items-center gap-2 md:flex-row md:gap-2">
<span className="text-sm font-semibold text-[#3498db] sm:text-base">{vendorName}</span>
...
<span className="rounded-full bg-[#27ae60] px-3 py-1.5 text-sm font-semibold text-white">{rating}</span>
<span className="text-sm text-gray-500">{reviews} reviews</span>
</div>
<div className="mb-3 flex flex-wrap items-center gap-1.5 md:flex-row md:gap-3">
<span className="text-xl font-bold text-[#2c3e50] sm:text-[22px]">₹{price}</span>
{originalPrice && (
<>
<span className="text-base font-medium text-gray-400 line-through">₹{originalPrice}</span>
<span className="text-base font-semibold text-[#27ae60]">{discountPercent}% off</span>
</>
)}
</div>
<p className="mb-4 text-sm text-gray-500">
<span className="font-semibold text-[#34495e]">Prep Time: </span>{duration}
</p>
</div>
