...
const VendorCalendar = ({ disabledDays = [], onDateSelect }) => {
  ...
  let footer = <p className='text-sm text-center py-2'>Please pick the first day of your event.</p>;
  if (selectedRange?.from) {
    if (!selectedRange.to) {
      footer = <p className='text-sm text-center py-2'>Now, please pick the last day.</p>;
    } else if (selectedRange.to) {
      footer = (
        <p className='text-sm text-center py-2 font-semibold'>
